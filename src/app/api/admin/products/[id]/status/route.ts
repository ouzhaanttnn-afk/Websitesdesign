...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await getProductById(id))) return NextResponse.json({ error: "Ürün bulunamadı." }, { status: 404 });
  ...
  const product = await setProductStockStatus(id, body.status as StockStatus);
